...
make mostlyclean
deps=`i686-w64-mingw32-objdump -p chronicle.exe | grep \.dll | awk '{print $3}'`
more_deps='libFLAC++-6.dll libvorbis-0.dll libFLAC-8.dll libogg-0.dll libsndfile-1.dll libspeex-1.dll libvorbis-0.dll libvorbisenc-2.dll libvorbisfile-3.dll libwinpthread-1.dll'
deps="$deps $more_deps"
#here=`pwd`
dep_paths=`python ../.ci/mingw32-32/list_full_deps.py $deps`
# for dep in ${deps}
# do
